...
export class QuerySubject<T> {
    ...
    public getSqlSelectParsed(sql?: IStatement): string {
        ...
        let selectSqlStatement = alasql
            .parse(querySelect + statement, params)
            .toString();
        if (params) {
        ...
        if (sql.where) {
            selectSqlStatement = querySelect + ' WHERE (' + ownerWhere + ') AND (' + selectSqlStatement.substr(querySelect.length + 6) + ')';
        } else {
