...
export class Repository<T> {
    ...
    public create(
        ...
        return new Promise<IEntity<T>>(async (resolve, reject) => {
            ...
            if (promises.length) {
                Promise.all(promises).then(() => resolve(c)).catch((e) => reject(e));
            } else {
                resolve(c);
            }
    ...
    public remove(data: IEntity<T> | T, skipConnector?: string): Promise<IEntity<T>> {
        return new Promise<IEntity<T>>((resolve, reject) => {
            ...
            if (promises.length) {
                Promise.all(promises).then(() => resolve(data as any)).catch((e) => reject(e));
            } else {
                resolve(data as any);
            }
    ...
    public createClassInstance(id?: string | number, data?: any): IEntity<T> {
        ...
        Object.defineProperty(c, 'revert', {
            ...
            configurable: false,
            writable: false,
            value: async (): Promise<IEntity<T>> => {
                const e = await this.create(JSON.parse(c['__initialdata']));
                this._subjects.forEach((subject: QuerySubject<T>) => {
                    subject.updateQueryCallbackChanges({dataAdded: true});
                });
                return e;
