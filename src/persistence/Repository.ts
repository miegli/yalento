...
export class Repository<T> {
    ...
    private cachedByIdentifier: { [uuid: string]: boolean } = {};
    ...
    public getOneByIdentifier(identifier: string): Promise<IEntity<T>> {
        return new Promise<IEntity<T>>(async (resolve) => {
            ...
            const existingData = this.cachedByIdentifier[identifier] ? await this.exec({
                where: 'uid LIKE ?',
                params: [identifier]
            }).then() : [];
            ...
            if (existingData.length) {
                ...
                Promise.all(promises).then(() => {
                    this.exec({where: 'uid LIKE ?', params: [identifier]}).then((data) => {
    ...
    public exec(sql: IStatement): Promise<Array<IEntity<T>>> {

        return new Promise<Array<IEntity<T>>>((resolve) => {
            const subject = new QuerySubject<T>(this, sql);
            subject.execStatement(sql, true).then((results: any) => {
                resolve(results);
            });
        })
    ...
    public create(
        ...
        return new Promise<IEntity<T>>(async resolve => {
            ...
            if (id && this.cachedByIdentifier[id]) {
                this.cachedByIdentifier[id] = false;
            } else if (data && this.cachedByIdentifier[data['__uuid']]) {
                this.cachedByIdentifier[data['__uuid']] = false;
            }
    ...
    public remove(data: IEntity<T>, skipConnector?: string): Promise<IEntity<T>> {
        return new Promise<IEntity<T>>(resolve => {
            ...
            if (this.cachedByIdentifier[data['__uuid']]) {
                this.cachedByIdentifier[data['__uuid']] = false;
            }
    ...
    public update(data: IEntity<T>, skipConnector?: string): Promise<IEntity<T>> {
        return new Promise<IEntity<T>>(resolve => {
            ...
            if (this.cachedByIdentifier[data.getUuid()]) {
                this.cachedByIdentifier[data.getUuid()] = false;
            }
