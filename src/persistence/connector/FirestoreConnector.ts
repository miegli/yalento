...
export class FirestoreConnector<T> extends AbstractConnector<T> {
    ...
    public add(items: Array<IEntity<T>>) {
        items.forEach((item: any) => {
            ...
            docReference.set(data, {merge: true})
                .then(() => {
                    ...
                    if (this.options.parent) {
                        ...
                        (this.db as any).doc(this.getParentDocumentPath()).set({__references: references}, {merge: true}).then().catch()
                    }
    ...
    public remove(items: Array<IEntity<T>>) {
        items.forEach((item: any) => {
            ...
            const docReference = (this.db as any)
                .doc(this.getPath() + '/' + item.__uuid);
            ...
            docReference.delete()
                .then(() => {
                    ...
                    if (this.options.parent) {
                        const references = {};
                        const documents = {};
                        documents[item.__uuid] = null;
                        references[this.repository.getClassName()] = {
                            documents: documents
                        };

                        (this.db as any).doc(this.getParentDocumentPath()).update({__references: references}).then().catch()
                    }
