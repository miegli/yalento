...
import {safeGet} from "../../../lib/lib/FireSQL/src/utils";
...
export class FirestoreConnector<T> extends AbstractConnector<T> {
    ...
    public async select(sql: string) {
        ...
        if (this.lastSql !== finalSql) {
            ...
            this.firestoreCollectionSnapshotUnsubscribe = this.getFirebaseCollection(finalSql).onSnapshot((querySnapshot: firebase.firestore.QuerySnapshot<firebase.firestore.DocumentData>) => {
                this.repository.createMany(querySnapshot.docs.map(value => value.data()) as any, '', 'firestore').then();
        ...
        const addQuery = (reference: firebase.firestore.Query, statement: IAstStatement): firebase.firestore.Query => {
            ...
            if (statement.operator === '=' || statement.operator === 'LIKE') {
                reference = reference.where(
                    statement.left.type === 'column_ref' ? statement.left.column : statement.right.column,
                    '==',
                    statement.left.value ? statement.left.value : statement.right.value
                );
            }

            if (statement.operator === '>' || statement.operator === '<' || statement.operator === '<=' || statement.operator === '>=') {
            ...
            if (statement.operator === 'IN') {
                reference = reference.where(
                    statement.left.type === 'column_ref' ? statement.left.column : statement.right.column,
                    'in',
                    (statement.left.value ? statement.left.value : statement.right.value).map(v => v.value)
                );
            }
