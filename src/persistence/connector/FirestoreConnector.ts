...
export class FirestoreConnector<T> extends AbstractConnector<T> {
    ...
    private async observeGeoLocation() {
        ...
        if (this.options.nearBy) {
            ...
            if (JSON.stringify([point.latitude, point.longitude, this.options.nearBy.radius.getValue()]) === this.lastGeoHash) {
                return;
            }
            ...
            if (this.geoFireClientExecutorApp) {
                ...
                this.geoFireClientExecutorApp = (firebase.initializeApp(this.app.options, 'geoFireClientExecutor') as any);
            }
            ...
            const collectionRef = this.geoFireClientExecutor
                .collection(this.getPath(), ref => {
                    return ref.where('__owner.EVERYBODY', '==', true);
                });

            this.rxQuerySubscriberGeoLocationQuery = collectionRef.within(point, this.options.nearBy.radius.getValue(), '__location');

            this.rxQuerySubscriberGeoLocation = this.rxQuerySubscriberGeoLocationQuery.pipe(takeUntil(this.rxQuerySubscriberGeoLocationQueryDestroy$), toGeoJSON('__location', true)).subscribe((e: any) => {
