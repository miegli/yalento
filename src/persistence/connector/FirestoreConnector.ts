    ...
    public async add(items: Array<IEntity<T>>) {
        items.forEach((item: any) => {
            if (!item['__timestamp'] || item['__timestamp'] > this.repository.getCreationTimestamp()) {
                ...
                if (this.debug) {
                    this.debugMessage(
                        `firebase skipped writing to ${this.getPath() + '/' + item.__uuid} while remote data is not synchronized.`,
                    );
                }
            }
        });
        ...
        if (this.lastSql !== finalSql) {
            ...
            this.firestoreCollectionSnapshotUnsubscribe = this.getFirebaseCollection(finalSql).onSnapshot(
                (querySnapshot: firebase.firestore.QuerySnapshot<firebase.firestore.DocumentData>) => {
                    ...
                    this.repository.createMany(querySnapshot.docs.map(value => value.exists ? value.data() : {
                        ...value.data(),
                        '__removed': true
                    }) as any, '', 'firestore').then();
