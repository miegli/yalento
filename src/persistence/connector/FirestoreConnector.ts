...
export class FirestoreConnector<T> extends AbstractConnector<T> {
    ...
    public async select(sql: string) {
        ...
        let finalSql = 'SELECT * FROM ' + this.getPath() + ' WHERE ' + this.replaceSql(originalSqlParts[1], !!(this.options.nearBy && this.options.nearBy.radius.getValue() > 0));
        ...
        if (this.options.nearBy && this.options.nearBy.radius.getValue() > 0) {
            ...
            finalSql += geoquery;
            this.repository.setGeoQuery(geoquery);
        } else {
            ...
            if (this.realtimeMode) {
                ...
                this.rxQuerySubscriber = data$.subscribe((results: any) => {
                    ...
                    if (hasGeoLocations && this.options.nearBy) {
                        this.repository.updateGeoLocations(results, this.options.nearBy.lat.getValue(), this.options.nearBy.long.getValue());
                    }
